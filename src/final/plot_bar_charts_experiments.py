...
def make_bar_plot_experiments(
        agg_level,
        data_exp_in,
        super_star_prices_2_firm,
        super_star_prices_3_firm,
        all_treatments):
    """
    ...
    for treatment in all_treatments:
        ...
        if treatment not in ['0H2A', '0H3A']:
            plotting_dict[treatment]['mean'].append(
                np.mean(
                    data_exp_in.loc[(data_exp_in['treatment'] == treatment)]
                    [agg_level]))
            ...
            plotting_dict[treatment]['mean'].append(
                super_star_prices_2_firm.mean())
            plotting_dict[treatment]['error_bar'].append(
                super_star_prices_2_firm.std())
        elif treatment == '0H3A':
            plotting_dict[treatment]['mean'].append(
                super_star_prices_3_firm.mean())
            plotting_dict[treatment]['error_bar'].append(
                super_star_prices_3_firm.std())
    ...
    rects = ax.patches
    all_means = list(np.array(
        [plotting_dict[treatment]['mean']
         for treatment in all_treatments]).flat)
    all_error = list(
        np.array(
            [plotting_dict[treatment]['error_bar']
             for treatment in all_treatments]).flat)
...
if __name__ == '__main__':
    ...
    make_bar_plot_experiments(
        agg_level='winning_price',
