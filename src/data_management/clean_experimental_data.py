...
def clean_session(session_data, super_group_ids, data_identifier):
    ...
    data_long['app'], data_long['round'], data_long['otree_object'], data_long['variable'] =  zip(*data_long['variable_long'].apply(lambda x: split_column(x)))
    data_long = data_long[['participant.code', 'app', 'round', 'variable', 'otree_object', 'value']].copy()
    
    # Get the super game indicator
    data_long['super_game'] = data_long['app'].transform(lambda x: app_to_sg(x))
    ...
    data_long = data_long.loc[data_long['otree_object'].isin(['group', 'player'])].copy()
    
    # Add the super game (App) specific group ID
    group_id_df = data_long.loc[data_long['variable'] == 'id_in_subsession'][['participant.code', 'app', 'value']].copy()
    group_id_df.rename({'value': 'group_id'}, inplace=True, axis=1)
    group_id_df = group_id_df.drop_duplicates().copy()
    data_long = data_long.merge(group_id_df, how = 'left', on=['participant.code', 'app'])
    ...
    data_merge_on = data_round_specific[['participant.code', 'round', 'super_game', 'group_id']].drop_duplicates().copy()
    
    # Add the super game and round specific price variables as columns 
    for var in ['winning_price', 'price', 'price_algorithm']:
    ...
    data_merge_on = merge_new_column_non_repeating(data_merge_on=data_merge_on,
                                                   data_merge_from=session_data,
                                                   var_name='introduction.1.group.group_treatment')
    data_merge_on.rename({'introduction.1.group.group_treatment': 'treatment'}, inplace=True, axis=1)

    # Add the super group id based on the group id
    data_merge_on['super_group_id'] = data_merge_on.apply(lambda x: add_super_group_id(individual_group_id = x['group_id'],
                                                                                       super_group_ids = super_group_ids,
                                                                                       treatment=x['treatment']), axis=1)
    
    # Get the size of the set of unique prices
    data_merge_on['unique_prices_humans'] = data_merge_on.groupby(['group_id', 'super_game', 'round']).transform('nunique')['price']
    ...
    data_merge_on['collusive'] = check_if_collusive(data_merge_on=data_merge_on)
    ...
    data_merge_on['super_group_id_general'] = data_merge_on['data_identifier'] + '_' + data_merge_on['super_group_id'].astype('str')
    data_merge_on['group_id_general'] = data_merge_on['data_identifier'] + '_' + data_merge_on['group_id'].astype('str')
    

    # Create lagged variables
    # TODO: Check this again.
    for ix_lag in range(5):
        data_merge_on['winning_price_lag_{}'.format(ix_lag)] = data_merge_on.groupby(['super_game', 'group_id', 'participant.code'])['winning_price'].shift(ix_lag)
        data_merge_on['price_lag_{}'.format(ix_lag)] = data_merge_on.groupby(['super_game', 'group_id', 'participant.code'])['price'].shift(ix_lag)
        data_merge_on['price_algorithm_{}'.format(ix_lag)] = data_merge_on.groupby(['super_game', 'group_id', 'participant.code'])['price_algorithm'].shift(ix_lag)
    ...
    # and round effects.
    data_merge_on['r_sg'] = data_merge_on['round'].astype(str) + data_merge_on['super_game'].astype(str)
...
if __name__ == '__main__':
    ...
    data_group_level = data_all.drop_duplicates(subset=['round', 'super_game', 'group_id_general', 'treatment']).copy()
    data_group_level = data_group_level[['round', 'super_game', 'r_sg', 'group_id_general', 
                                         'treatment', 'winning_price', 'collusive',
                                         'super_group_id_general', 'price_algorithm']]
    data_group_level.to_pickle(ppj("OUT_DATA", "data_group_level.pickle"))
    data_group_level.reset_index().to_feather(ppj("OUT_DATA", "data_group_level.feather"))
